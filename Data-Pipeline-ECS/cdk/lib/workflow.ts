...
export function createDataPipelineStateMachine(
    ...
    bucket: string)
    {
      // Broadcast Status task and parallel state definitions
      ...
      // Configurations for the ECS Task to run
      ...
      // Add catch statements to perform different actions based on exception. Some exceptions are from the state machine execution and some could be from the task being run. Here we send all errors to EventBridge
      ...
      // Sometimes tasks can take longer than expected (they timeout!) and need to be investigated to determine success or failure. Send notifications to developers to investigate result and retry separately if required
      .addCatch(broadcastErrorStatus, {
